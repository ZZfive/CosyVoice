...
def main():
    ...
    with open(args.config, 'r') as f:
        configs = load_hyperpyyaml(f, overrides=override_dict)  # 加载配置文件， override_dict用于将不需要初始的部分用None覆盖
    if gan is True:
    ...
    model = wrap_cuda_model(args, model)  # 将模型从CPU转移到GPU

    # Get optimizer & scheduler
    model, optimizer, scheduler, optimizer_d, scheduler_d = init_optimizer_and_scheduler(args, configs, model, gan)  # 初始化优化器和学习率调度器   
    scheduler.set_step(start_step)  # 设置调度器步数
    if scheduler_d is not None:
        scheduler_d.set_step(start_step)

    # Save init checkpoints
    info_dict = deepcopy(configs['train_conf'])  # 复制初始训练配置
    info_dict['step'] = start_step
    info_dict['epoch'] = start_epoch
    save_model(model, 'init', info_dict)  # 保存初始模型

    # Get executor
    executor = Executor(gan=gan)  # 初始化执行器
